...
export setmdlparams!
export SingleKernelRegressor
export MultipleKernelRegressor
export KernelFunctions
export PnormKernelStd
export datakernelization
...
#
###############################################################################
#tosymboldict is pulled from DrWatston.jl.  
"""
tosymboldict(d)

Change a dictionary with key type `String` to have key type `Symbol`.

This function is pulled for DrWatson.jl source
"""
tosymboldict(::Type{DT},d) where {DT<:AbstractDict} = DT(zip(Symbol.(keys(d)), values(d)))
tosymboldict(d) = tosymboldict(Dict,d)

"""
setmdlparams!(mdl, params::Dict)

Cycles over keys in params dictionary and sets property of mdl
that matches. "mdl" is MLJ model.
"""
function setmdlparams!(obj, params::Dict)
  #see if any params match obj fields
  for pkeys in keys(tosymboldict(params))
    if any(pkeys.==propertynames(obj))
      setfield!(obj, pkeys, params[String(pkeys)])
    end
  end

  #recurse deeper if composite model (SK or MK)
  if any(propertynames(obj) .== :mdl)
    for pkeys in keys(tosymboldict(params))
      if any(pkeys.==propertynames(obj.mdl))
        setfield!(obj.mdl, pkeys, params[String(pkeys)])
      end
    end #end recursive loop
  end #end recursive loop for mdl params
end #end of function
...
Input is vector of eigenvalues.  Calculate the effective rank of a matrix
(spectral entropy).

O. Roy and M. Vetterli, "The effective rank: A measure of effective 
dimensionality," 2007 15th European Signal Processing Conference, 2007, 
pp. 606-610.
"""
function effrank(λ_vec::AbstractVector)
  #normalize eigenvalue vector (L1 norm)
  λ_norm = λ_vec./norm(λ_vec,1)

  #return effective rank
  return exp(-1.0*sum(λ_norm.*log.(λ_norm)))
end #effrank
...
function datakernelization(Xref, p_kern::Float64; λ_kern::Float64=999.0)
    """
    DataKernelization(Xref, p_kern::Float64, λ_kern::Float64=999.0)

    Kernelizes data into higher dimensional space using Minkowski function which
    is parameterized by the "p_kern" which determines L_p space.  It is scaled
    by the "λ_kern" parameter unless defautl 999.0 which means resulting kernel
    matrix is normalized by it's own standard deviation (and the λ_kern is udpated
    to that value).

    The λ_kern is returned as used to scale kernelization of future data

    returns kernel matrix (matrix), update λ_kern (float scalar)
    """

    #default behaviour is to normalize λ_kern to std dev of kernel matrix
    if λ_kern==999.0
        dKt = KernelFunctions.compose(PnormKernelStd(p_kern), ScaleTransform(1))
        dKKt=kernelmatrix(dKt, Tables.matrix(Xref), obsdim=1)
        λ_kern=1.0/(Statistics.std(dKKt))
    end
 
    #employ kernel trick into vectored space
    return PnormKernelFunction(Xref, p=p_kern, λ_kern=λ_kern), λ_kern

end #datakernelization method one

function datakernelization(Xnew, Xref, p_kern::Float64, λ_kern::Float64=999.0)
    """
    DataKernelization(Xnew, Xref, p_kern::Float64, λ_kern::Float64)

    Kernelizes data into higher dimensional space using Minkowski function which
    is parameterized by the "p_kern" which determines L_p space.  It is scaled
    by the "λ_kern" parameter unless set to 999.0 which means resulting kernel
    matrix is normalized by it's own standard deviation. In general, try to avoid 
    recalculating if already did as part of single method

    returns kernel matrix (matrix)
    """

    #default behaviour is to normalize λ_kern to std dev of kernel matrix
    #should avoid recalculating this as only needed once
    if λ_kern==999.0
        dKt = KernelFunctions.compose(PnormKernelStd(p_kern), ScaleTransform(1))
        dKKt=kernelmatrix(dKt, Tables.matrix(Xref), obsdim=1)
        λ_kern=1.0/(Statistics.std(dKKt))
    ...
    #note that converted from table/dataframe to matrix and back to table
    return PnormKernelFunction(Xnew, Xref, p=p_kern, λ_kern=λ_kern)
...
function MMI.fit(m::SingleKernelRegressor, verbosity::Int, X, y, w=nothing)
    ...
    #default behaviour is to normalize λ_kern to std dev of kernel matrix
    @debug "TID:"*string(Threads.threadid())*": normalizing fit data..."
    if m.λ_kern==999.0
    ...
    @debug "TID:"*string(Threads.threadid())*": done normalizing fit data..."
    
    #employ kernel trick into vectored space
    @debug "TID:"*string(Threads.threadid())*": kernel trick..."
    ...
    @debug "TID:"*string(Threads.threadid())*": done kernel trick..."
    ...
    @debug "TID:"*string(Threads.threadid())*": internal fitting..."
    core_fitresult = MLJ.fit!(mach, verbosity=2)
    @debug "TID:"*string(Threads.threadid())*": done internal fitting..."
...
function MMI.predict(m::SingleKernelRegressor, fitresult, Xnew)
    ...
    if w!=nothing K_pred = MLJ.table(matrix(K_pred).*w') end
